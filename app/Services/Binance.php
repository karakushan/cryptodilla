...
use App\Events\ExchangeTradesEvent;
...
class Binance implements ExchangeInterface
{
    ...
    {
        $info = Cache::remember('binance_exchange_info', 3600, function () {
        ...
        $symbols = array_filter($symbols, function ($item) {
            return $item['isSpotTradingAllowed'] === true && in_array('SPOT',$item['permissions']);
        });
    ...
    {
        $this->api->ticker($symbol, function ($api, $symbol, $ticker) {
            ExchangeTickerEvent::dispatch($ticker);
        });
    }

    public function trades($symbol, $limit = 10)
    {
        $this->api->depthCache([$symbol], function ($api, $symbol, $depth) use ($limit) {
            $sorted = $this->api->sortDepth($symbol, $limit);
            ExchangeTradesEvent::dispatch($sorted);
        });
    }
